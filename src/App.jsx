...
function App() {
  ...
  // 60FPS Unified RAF Physics + Render Engine (12x12 High Frame Rate Grid)
  useEffect(() => {
  ...
  // Touch Swipe & Tap Handlers for Snake (Both screen swipe and tap steering)
    ...
    const renderRhythmLoop = (time) => {
      try {
        ...
        for (let l = 0; l < 4; l++) {
          ...
          if (pressAge < 200) {
            const beamAlpha = (1 - pressAge / 200) * 0.35;
        ...
        // 2. 4 个打击靶位底座（通透温润白瓷质感，带中心定位槽，叠放在实线上方）
        for (let l = 0; l < 4; l++) {
          const margin = 2.5;
          ...
          if (isPressed) {
            ...
            idleGrad.addColorStop(0.5, 'rgba(255, 255, 255, 0.92)');
            idleGrad.addColorStop(1, 'rgba(241, 245, 249, 0.90)');
          ...
          ctx.strokeStyle = isPressed ? 'rgba(255, 255, 255, 0.95)' : 'rgba(255, 255, 255, 0.90)';
          ctx.lineWidth = 1.5;
          ctx.stroke();
          ...
          ctx.strokeStyle = isPressed ? 'rgba(255, 255, 255, 0.90)' : 'rgba(148, 163, 184, 0.70)';
          ctx.lineWidth = 2.5;
          ctx.lineCap = 'round';
          ctx.stroke();
        ...
        waves.forEach(w => {
          if (w.resolved || w.t < -2.0 || w.t > progressToEnd + 2.0) return;
      ...
      <main className="main-content" ref={mainContentRef} style={{ marginTop: `${calculatedMarginTop}px` }}>
        {searchQuery.trim() !== '' ? (
          ...
          <div className="snake-game-stage">
            ...
            {/* 60FPS Canvas 渲染竞技场 (全宽正方形主舞台，支持屏幕任意滑动与轻触) */}
            <div 
          ...
          <div className="rhythm-game-stage">
            ...
            {/* 60FPS 4 轨道 Canvas 渲染主舞台 (全沉浸无界 3D 跑道) */}
            <div 
            ...
            {/* ⚡ 移速滑块控制调节条 */}
            <div className="rhythm-speed-slider-bar">
              <div className="speed-slider-label">
                <span className="speed-slider-title">⚡ 移速设定</span>
                <span className="speed-level-badge">
